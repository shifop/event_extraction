...
def get_dataset_by_id(path, batch_size, vocab, max_length, isdev = False):
    ...
    for xi, x in enumerate(data):
        content, group = x
        start_tag, end_tag, entity_index, entity_map, entity_type = process_entity([group], len(content) + 2)

        content_ids = vocab.convert_tokens_to_ids(
            ['[CLS]']+[_ if _ in vocab.vocab else '[UNK]' for _ in content]+['[SEP]'])

        cache.append([' '+content+' ', len(content_ids), padding2(content_ids, 0, max_length),
...
#
# for index, x in enumerate(get_dataset_by_id(['./data/process/train.json'], 4, vocab, max_length)()):
#     print(index)
#     i = 1
#
# print('')
...
def get_dev_data(path, batch_size, vocab, max_length):
    cache = read_json(path)

    data = []
    for xi, x in enumerate(cache):
        content = x['sentence']

        content_ids = vocab.convert_tokens_to_ids(
            ['[CLS]'] + [_ if _ in vocab.vocab else '[UNK]' for _ in content] + ['[SEP]'])

        data.append([x, ' ' + content + ' ', len(content_ids), padding2(content_ids, 0, max_length)])
    ...
    def get_dataset():
        ...
        for i in range(size):
            ...
            for xi, x in enumerate(pdata):
                adata, content_path, mask, content_ids = x
            ...
            yield test_data, raw, content_ids_all, masks

    return get_dataset


if __name__=="__main__":
    name = ['bert_base_trigger','roberta_large_trigger', 'albert_xlarge_trigger']
    gpu_id = '1'
    ...
    for path in name:
        ...
        # 实际需要预测数据
        dev_data = get_dev_data('./data/raw/测试集/sentences.json', 4, vocab, max_length)
        ...
        for x in tqdm(test_data):
            ...
            for _ in rt_:
                trigger.append([])
                for __ in _:
                    trigger[-1].append([''.join(__[0]), __[1][0]-1, len(__[0])])

            for i in range(len(test_data_)):
                test_data_[i]['trigger'] = trigger[i]
            rt.extend(test_data_)
